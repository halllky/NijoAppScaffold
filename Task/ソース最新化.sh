...
for d in "$SCRIPT_DIR/../demo/"*; do
    if [ -d "$d" ]; then
        ...
        if [ $? -eq 0 ]; then
            # React
            cp "$SOURCE_ROOT/client/src/callComplexPostEndpointAsync.ts" "$TARGET_ROOT/client/src/callComplexPostEndpointAsync.ts"

            # Core
            rsync -av --delete --exclude=".*" "$SOURCE_ROOT/Core/Debugging/" "$TARGET_ROOT/Core/Debugging/"
            ...
            # Core.AutoGenerated 特になし

            # WebApi
            rsync -av --delete --exclude=".*" "$SOURCE_ROOT/WebApi/Base/" "$TARGET_ROOT/WebApi/Base/"
            cp "$SOURCE_ROOT/WebApi/Properties/launchSettings.json" "$TARGET_ROOT/WebApi/Properties/launchSettings.json"
            cp "$SOURCE_ROOT/WebApi/Properties/PublishProfiles/本番用ビルド.xml" "$TARGET_ROOT/WebApi/Properties/PublishProfiles/本番用ビルド.xml"
            ...
            # Task
            rsync -av --delete --exclude=".*" "$SOURCE_ROOT/Task/" "$TARGET_ROOT/Task/"
        fi
    fi
done

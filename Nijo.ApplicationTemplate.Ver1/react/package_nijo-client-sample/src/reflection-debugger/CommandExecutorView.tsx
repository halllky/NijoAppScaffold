...
const useCommandExecutor = (
  ...
  const executeCommand = React.useCallback(async (parameter: object): Promise<ExecutionHistory | undefined> => {
    ...
    // コマンド実行
    const result = await complexPost<object>(executionEndPoint, parameter)
    if (!result.ok && result.type === 'canceled') {
      // キャンセルされた場合は履歴に追加しない
      return undefined

    } else if (!result.ok && result.type === 'detail') {
      // パラメータエラー
      newHistory = {
        id: newHistoryId,
        parameter,
    ...
    } else if (!result.ok && result.type === 'unknown') {
      // 不明なエラー
      newHistory = {
        id: newHistoryId,
        parameter,

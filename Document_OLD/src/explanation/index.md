...
Explanationセクションは、NijoAppScaffoldの概念、設計思想、アーキテクチャを深く理解するためのドキュメントです。なぜこのフレームワークが必要なのか、どのような思想で設計されているのかを学べます。

## アーキテクチャ概念

### [モデル間の協調によるアプリケーション構成](./models-overview.md)
3つのモデルが**どのように協調するか**：具体的なデータフロー、協調例、設計考慮点

### [モデル設計思想と開発手法比較](./model-design-philosophy.md)
**なぜこの設計なのか**：背景思想、他手法との比較、アーキテクチャ設計原則の詳細

## 基本概念

### なぜNijoAppScaffoldが必要なのか？
...
これらの課題を解決するため、NijoAppScaffoldでは単一のスキーマ定義から全ての技術スタックに対応したコードを自動生成するアプローチを採用しています。
...
NijoAppScaffoldの拡張は、段階的なアプローチを推奨します：

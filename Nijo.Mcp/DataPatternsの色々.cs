...
partial class NijoMcpTools {
    ...
    public static async Task<bool> アプリケーションテンプレートを自動テストで作成されたプロジェクトにコピーする(
        ...
        var pagesDirectory = Path.Combine(DATA_PATTERN_REVEALED_DIR, "react", "apps", "autogenerated-app", "src", "pages");
    ...
    private static bool AIエージェントの実装成果をスナップショットフォルダに保存する(string dataPatternsXmlFileFullPath, string snapshotDir) {
        ...
        var destinationPagesDir = Path.Combine(snapshotDir, "react", "apps", "autogenerated-app", "src", "pages");
        ...
        // Reactルーティング
        File.Copy(
            Path.Combine(DATA_PATTERN_REVEALED_DIR, "react", "apps", "autogenerated-app", "src", "pages", "index.tsx"),
            Path.Combine(destinationPagesDir, "index.tsx"),
            overwrite: true);

        // React画面
        foreach (var sourceDirectory in Directory.GetDirectories(Path.Combine(DATA_PATTERN_REVEALED_DIR, "react", "apps", "autogenerated-app", "src", "pages"))) {

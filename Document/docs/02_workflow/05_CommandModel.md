...
引数や戻り値が必要な場合は、**DataModel**、**QueryModel**、または **StructureModel** で定義された構造を指定して利用します。

* **DataModel**: データの登録・更新処理などで、永続化対象のデータ構造をそのまま受け渡す場合に使用します。
* **QueryModel**: 検索条件を受け取る場合などに使用します。
* **StructureModel**: DB保存も検索もしない、純粋なデータの受け渡し用の構造体です。画面固有の入力フォームや、複雑な計算結果の返却などに使用します。

参照する場合、用途に応じて以下の形式を選択します（nijo.xmlでの定義時に指定）。

* **DisplayData**: 全項目（表示用データ）として展開されます。確認画面や結果表示に適しています。
* **SearchCondition**: 検索条件オブジェクトとして展開されます。
* **Key**: 主キーのみとして展開されます。単に紐付けを行う場合（IDだけ渡せば良い場合）に適しています。

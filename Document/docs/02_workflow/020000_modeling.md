...
典型的な例として、よくある一覧検索画面と詳細編集画面の例を挙げます。
デモ101では「売上一覧画面」「売上詳細画面（新規登録または更新を行う画面）」があり、この画面では以下のモデルを利用して実現しています。

一覧検索画面では1つの Query Model を、詳細編集画面では、初期表示・新規登録・更新の3つの Command Model を利用しています。
それら4つの Query / Command Model 内部では、売上・商品・従業員といった Data Model のモジュールを利用して処理を行っています。

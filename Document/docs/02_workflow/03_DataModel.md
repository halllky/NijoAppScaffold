...
import NijoSchemaViewer from "@site/src/components/NijoSchemaViewer";
...
<NijoSchemaViewer nijoXmlPath="demo/000_基本的なDataModelの例/nijo.xml" />

この定義から以下のソースコードが生成されます。

<SourceCodeViewer height="640px" project="demo/000_基本的なDataModelの例" files={[{
  path: "Core.AutoGenerated/__AutoGenerated/Util/AutoGeneratedDbContext.cs",
  comment: "自動生成されたDbContextクラス",
  focus: 8,
}, {
  path: "Core.AutoGenerated/__AutoGenerated/社員/Class_EFCoreEntity.cs",
  comment: "Entity Framework Core エンティティ定義",
  focus: 21,
},{
  path: "Core.AutoGenerated/__AutoGenerated/社員/バリデーション処理.cs",
  comment: "必須チェック、桁数チェックなど自動生成可能なバリデーション",
  focus: 20,
},{
  path: "Core.AutoGenerated/__AutoGenerated/社員/新規登録処理.cs",
  comment: "INSERT",
  focus: 20,
},{
  path: "Core.AutoGenerated/__AutoGenerated/社員/更新処理.cs",
  comment: "UPDATE",
  focus: 20,
},{
  path: "Core.AutoGenerated/__AutoGenerated/社員/物理削除処理.cs",
  comment: "DELETE",
  focus: 20,
}]} />
...
* **物理削除処理 (`Delete...`)**: データを物理削除するメソッド。オプションで論理削除（削除データ専用の別テーブルに逃がす形）に変更可能。
...
国コード、通貨単位、分析集計表の分類など、システムの運用中に値の追加削除が発生しうる区分が格納される。
明示的に指定したデータに対してのみ自動生成される。

詳細は [汎用参照テーブルのドキュメント](./03_DataModel_GenericLookupTable.md) を参照。

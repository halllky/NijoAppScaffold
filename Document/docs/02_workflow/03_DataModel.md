...
| モジュール | 詳細ページ |
| --- | --- |
| EF Core エンティティクラス・DbContext 設定 | [EF Core エンティティ定義](./DataModel_EFCoreEntity) |
| CRUD メソッド（新規登録・更新・物理削除） | [CRUD メソッド](./DataModel_CRUDMethods) |
| 入力バリデーション（必須・最大長・文字種・桁数） | [バリデーション属性リファレンス](./DataModel_Validators) |
| 論理削除メソッド（オプション） | [論理削除](./DataModel_SoftDelete) |
| 一括更新 API（オプション） | [一括更新](./DataModel_BatchUpdate) |
| 汎用参照テーブル（オプション） | [汎用参照テーブル](./DataModel_GenericLookupTable) |
| ダミーデータ生成処理 | [ダミーデータ生成](./DataModel_DummyData) |

...
多くの場合は自動生成されたエンティティ定義で事足りますが、
追加の設定が必要な場合は `OnModelCreating` メソッドをオーバーライドしてください。

```cs
public partial class OverridedDbContext // <- 手動実装の DbContext
    : AutoGeneratedDbContext {          // <- 自動生成された DbContext

    protected override void OnModelCreating(ModelBuilder modelBuilder) {

        // 【重要】自動生成されたモデル定義はそのまま援用することを推奨
        base.OnModelCreating(modelBuilder);

        // 自動生成で賄えないDB定義編集を追加する。
        // たとえば主キー以外のインデックス定義など。
        modelBuilder.Entity<MyTableDbEntity>(e => {
            e.HasIndex(x => new {
                x.MyColumn01,
                x.MyColumn02,
            });

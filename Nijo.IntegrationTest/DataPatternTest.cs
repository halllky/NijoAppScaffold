...
public class DataPatternTest {
    ...
    public void Setup() {
        ...
        foreach (var dir in Directory.GetDirectories(Path.Combine(testProjectDir, "react", "apps", "autogenerated-app", "src", "pages"))) {
            Directory.Delete(dir, true);
        }
        // テンプレートプロジェクトでしか使わないソースを削除（ルーティング。pages/index.tsx）
        File.WriteAllText(Path.Combine(testProjectDir, "react", "apps", "autogenerated-app", "src", "pages", "index.tsx"), $$"""
            /** 画面のルーティング設定 */

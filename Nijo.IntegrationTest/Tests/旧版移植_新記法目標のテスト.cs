...
public class 旧版移植_新記法目標のテスト {
    ...
    [Test]
    public async Task ReadModel2の添付ファイルディープイコールが旧版と同じソースを生成すること() {
        using var legacyProject = await NijoTestUtil.CreateLegacyProjectAsync("""
          <LegacyApp UseWijmo="True"
                 DisableLocalRepository="True"
                 UseBatchUpdateVersion2="True"
                 ...
                 MaxTotalFileSizeMB="10"
                 AttachmentFileExtensions="doc;docx;xls;xlsx;pdf">
            <添付照会 is="read-model-2"
              LatinName="attachment_query"
              DisplayName="添付照会">
            <添付照会ID is="int:9 key" />
            <添付資料 is="file" />
            </添付照会>
        ...
        using var migratedProject = await NijoTestUtil.CreateNewProjectAsync("""
          <NijoAppScaffold UseWijmo="True"
                   ...
                   ReactProjectFolderName="react/src">
            <DataStructures>
            <添付照会 Type="read-model-2"
                LatinName="attachment_query"
                DisplayName="添付照会">
              <添付照会ID Type="int" TotalDigit="9" IsKey="True" />
              <添付資料 Type="file" />
            </添付照会>
            </DataStructures>
          </NijoAppScaffold>
        ...
        using var migratedProject = await NijoTestUtil.CreateNewProjectAsync($$"""
            <NijoAppScaffold UseWijmo="True"
              ...
              <GenericLookupTableCategories>
                <Categories For="compat-glt-申請区分マスタ">
                  <application-kind DisplayName="申請区分種別" />
                </Categories>
              </GenericLookupTableCategories>

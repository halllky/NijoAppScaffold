...
namespace Nijo {
    public class Program {
        ...
        private static RootCommand DefineCommand(CancellationTokenSource cancellationTokenSource) {
            ...
            var dumpEnum = new Option<bool>(
                name: "--enum",
                description: "このスキーマで定義されている静的区分/動的区分をTSV形式でダンプします。他のオプションと併用することはできません。");
            ...
            var dump = new Command(
                ...
                { verbose, path, mermaid, dumpEnum };
            dump.SetHandler((verbose, path, mermaid, dumpEnum) => {
                var logger = ILoggerExtension.CreateConsoleLogger(verbose);
                var schema = GeneratedProject
                    .Open(path, serviceProvider, logger)
                    .BuildSchema();
                if (dumpEnum) {
                    schema.DumpEnumDefTsv(Console.Out);
                } else if (mermaid) {
            ...
            var kbn = new Command(
                name: "kbn",
                description: "このスキーマで定義されている静的区分・動的区分をTSV形式で列挙します。")
                { path };
            kbn.SetHandler((path) => {
                ...
                var schema = GeneratedProject
                    ...
                    .BuildSchema();
                // 静的区分
                Console.WriteLine($"静的区分\t表示名\t物理名\t値");
                foreach (var def in schema.EnumDefinitions.OrderBy(x => x.Name)) {
                    foreach (var val in def.Items.OrderBy(x => x.Value)) {
                        Console.WriteLine($"{def.Name}\t{val.DisplayName}\t{val.PhysicalName}\t{val.Value}");
                    }
                    Console.WriteLine($"===");
                }
                // 動的区分
                Console.WriteLine();
                Console.WriteLine($"【動的区分表示名】");
                foreach (var def in schema.DynamicEnumTypeInfo.OrderBy(x => x.DisplayName)) {
                    Console.WriteLine($"{def.DisplayName}");
                }
            }, path);
            rootCommand.AddCommand(kbn);

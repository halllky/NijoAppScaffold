...
namespace Nijo.Models.CommandModelFeatures {
    ...
    internal class CommandParameter {
        ...
        private const string UNIQUE_ID_CS = ReadModel2Features.DataClassForDisplay.UNIQUE_ID_CS;
        private const string UNIQUE_ID_TS = ReadModel2Features.DataClassForDisplay.UNIQUE_ID_TS;
        ...
        internal string RenderCSharpDeclaring(CodeRenderingContext context) {
            return EnumerateThisAndDescendants().SelectTextTemplate(param => $$"""
                ...
                {{If(param._aggregate.IsChildrenMember(), () => $$"""
                    /// <summary>
                    ...
                    public required string {{UNIQUE_ID_CS}} { get; set; }
                """)}}
            ...
            return EnumerateThisAndDescendants().SelectTextTemplate(param => {
                ...
                return $$"""
                    ...
                    """)}}
                        ...
                        public {{param.MessageDataCsClassName}}({{DisplayMessageContainer.INTERFACE}} origin) : base(origin) {
                    {{members.SelectTextTemplate(m => $$"""
                ...
                string RenderConstructor(AggregateMember.AggregateMemberBase member, bool isTransferRendering) {
                    ...
                    if (member is AggregateMember.ValueMember || member is AggregateMember.Ref) {
                        ...
                        if (isTransferRendering) {
                            return $$"""
                                {{member.MemberName}} = new(origin, rowIndex => {
        ...
        internal string RenderTsNewObjectFunction(CodeRenderingContext context) {
            ...
            return aggregates.SelectTextTemplate(p => $$"""
                ...
                {{If(p._aggregate.IsChildrenMember(), () => $$"""
        ...
        internal class Member {
            ...
            internal string CsErrorMemberType => MemberInfo switch {
                AggregateMember.ValueMember => DisplayMessageContainer.INTERFACE,
                ...
                AggregateMember.Ref => DisplayMessageContainer.INTERFACE,
                _ => throw new NotImplementedException(),
            };

...
namespace Nijo.Models.WriteModel2Features {
    ...
    internal class DataClassForSave {
        ...
        internal IEnumerable<AggregateMember.AggregateMemberBase> GetOwnMembers() {
            return _ownMembersCache ??= _aggregate
                .GetMembers(flags: AggregateMember.E_GetMembersFlag.ValueMember | AggregateMember.E_GetMembersFlag.Ref | AggregateMember.E_GetMembersFlag.Child)
                .Where(m => m.DeclaringAggregate == _aggregate)
                .ToArray();
        }
        private AggregateMember.AggregateMemberBase[]? _ownMembersCache;
        ...
        private string RenderToDbEntity() {
            ...
            const string ARG_CURRENT_TIME = "currentTime";
            const string ARG_CURRENT_USER = "currentUser";
                    ...
                    if (member is AggregateMember.ValueMember vm) {
                        ...
                        yield return $$"""
                            {{children.MemberName}} = {{instanceName}}.{{member.GetFullPathAsForSave(since: instanceAgg).Join("?.")}}?.Select({{loopVar}} => new {{dbEntity.ClassName}} {
                                ...
                                {{If(Type == E_Type.Create, () => $$"""
                                {{EFCoreEntity.CREATE_USER}} = {{ARG_CURRENT_USER}},
                                    {{EFCoreEntity.UPDATE_USER}} = {{ARG_CURRENT_USER}},
                                    {{EFCoreEntity.CREATED_AT}} = {{ARG_CURRENT_TIME}},
                                    {{EFCoreEntity.UPDATED_AT}} = {{ARG_CURRENT_TIME}},
                                """)}}
            ...
            return $$"""
                ...
                /// </summary>
                public {{returnType}} {{TO_DBENTITY}}(string? {{ARG_CURRENT_USER}} = null, DateTime? {{ARG_CURRENT_TIME}} = null) {
                        ...
                        IEnumerable<string> RenderRefTargetKeyBody(DataClassForRefTargetKeys refTargetKey) {
                            foreach (var fk in refTargetKey.GetValueMembers()) {
                                ...
                                var vm = writeModel._aggregate
                                    .GetMembers(flags: AggregateMember.E_GetMembersFlag.ValueMember)
        ...
        internal string RenderCSharpMessageStructure(CodeRenderingContext context) {
            ...
            static string RenderConstructor(AggregateMember.AggregateMemberBase member, bool isConcreteRendering) {
                if (member is AggregateMember.ValueMember || member is AggregateMember.Ref) {
                    var value = isConcreteRendering
                        ? $"new {DisplayMessageContainer.CONCRETE_CLASS}([.. path, \"{member.MemberName}\"])"
                        : $"origin";
                    return $$"""
                    ...
                    var arrayArgs = isConcreteRendering
                        ? $"[.. path, \"{member.MemberName}\"]"
                        : $"origin";
                    var childArgs = isConcreteRendering
                        ? $"[.. path, \"{member.MemberName}\"], i"
                        : $"origin";
                    return $$"""
                        {{member.MemberName}} = new {{DisplayMessageContainer.CONCRETE_CLASS_LIST}}<{{desc.MessageDataCsInterfaceName}}>({{arrayArgs}}, i => {
                            return new {{desc.MessageDataCsClassName}}({{childArgs}});
                        });
                        """;
                    ...
                    var value = isConcreteRendering
                        ? $"new {desc.MessageDataCsClassName}([.. path, \"{member.MemberName}\"])"
                        : $"origin";
                    return $$"""
                        {{member.MemberName}} = {{value}};
                        """;

        ...
        private static IEnumerable<string> RenderAggregate(GraphNode<Aggregate> renderingAggregate, string instance, GraphNode<Aggregate> instanceAggregate) {
            foreach (var member in renderingAggregate.GetMembers()) {
                ...
                if (member is AggregateMember.Schalar schalar) {
                    ...
                    yield return $$"""
                        if ({{instance}}.{{path.Join("?.")}}{{cast}} != null &&
                            {{instance}}.{{path.Join("?.")}}{{cast}}.Length > {{schalar.Options.MaxLength}}) {
                            var 特殊文字リスト = DotnetExtensions.PickupMultipleCodeUnitCharacters({{instance}}.{{path.Join("?.")}}{{cast}}).ToArray();

                            if (特殊文字リスト.Length > 0) {
                                e.{{GetErrorMemberPath(member).Join(".")}}.AddError($"{{schalar.Options.MaxLength}}文字以内で入力してください。（２文字以上とカウントされる特殊文字が入っています[{string.Join(",", 特殊文字リスト)}]）");
                            } else {

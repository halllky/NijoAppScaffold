        ...
        private UtilityClass.CustomJsonConverter RenderJsonDeserialization() => new UtilityClass.CustomJsonConverter {
            ...
            ConverterClassDeclaring = $$"""
                            ...
                            if (addOrModOrDel == "ADD") {
                                return new {{CREATE_COMMAND}}<{{x.Create.CsClassName}}> {
                                    {{VALUES_CS}} = JsonSerializer.Deserialize<{{x.Create.CsClassName}}>(value.GetRawText(), options)
                                        ?? throw new InvalidOperationException({{MessageConst.CS_CLASS_NAME}}.{{MessageConst.C_INF0036}}("{{x.Create.CsClassName}}",value.GetRawText())),
                                };
                                ...
                                return new {{UPDATE_COMMAND}}<{{x.Upd.CsClassName}}> {
                                    {{VALUES_CS}} = JsonSerializer.Deserialize<{{x.Upd.CsClassName}}>(value.GetRawText(), options)
                                        ?? throw new InvalidOperationException({{MessageConst.CS_CLASS_NAME}}.{{MessageConst.C_INF0037}}("{{x.Upd.CsClassName}}",value.GetRawText())),
                                ...
                                return new {{DELETE_COMMAND}}<{{x.Upd.CsClassName}}> {
                                    {{VALUES_CS}} = JsonSerializer.Deserialize<{{x.Upd.CsClassName}}>(value.GetRawText(), options)
                                        ?? throw new InvalidOperationException({{MessageConst.CS_CLASS_NAME}}.{{MessageConst.C_INF0038}}("{{x.Upd.CsClassName}}",value.GetRawText())),
                                    {{VERSION_CS}} = jsonDocument.RootElement.GetProperty("{{VERSION_TS}}").GetInt32(),
                                };
                            } else if (addOrModOrDel == "NONE") {
                                return new {{NO_OPERATION}}<{{x.Upd.CsClassName}}> {
                                    {{VALUES_CS}} = JsonSerializer.Deserialize<{{x.Upd.CsClassName}}>(value.GetRawText(), options)
                                        ?? throw new InvalidOperationException({{MessageConst.CS_CLASS_NAME}}.{{MessageConst.C_INF0039}}("{{x.Upd.CsClassName}}",value.GetRawText())),
                                    {{VERSION_CS}} = jsonDocument.RootElement.GetProperty("{{VERSION_TS}}").GetInt32(),
                                };
                            }
                        }
                """)}}
                        ...
                        throw new InvalidOperationException({{MessageConst.CS_CLASS_NAME}}.{{MessageConst.C_INF0040}}(jsonDocument.RootElement.GetRawText()));
                    }

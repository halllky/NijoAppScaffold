...
namespace Nijo.Models.RefTo {
    ...
    internal class RefSearchMethod {
        ...
        private string AppSrvValidateMethod => $"Validate{_aggregate.Item.PhysicalName}RefSearchCondition";
            ...
            return $$"""
                ...
                public virtual IActionResult Load{{_aggregate.Item.PhysicalName}}(ComplexPostRequest<{{searchCondition.CsClassName}}> request) {
                    try {
                        ...
                        _applicationService.Log.Debug("Load(ref) {{_aggregate.Item.DisplayName.Replace("\"", "\\\"")}}: {0}", request.Data.ToJson());
                        throw;
                    }
                }
                ...
                public virtual IActionResult Count{{_aggregate.Item.PhysicalName}}(ComplexPostRequest<{{searchCondition.CsFilterClassName}}> request) {
                    try {
                        ...
                        _applicationService.Log.Debug("Count(ref) {{_aggregate.Item.DisplayName.Replace("\"", "\\\"")}}: {0}", request.Data.ToJson());
                        throw;
                    }
                }
                """;
        ...
        internal string RenderAppSrvMethodOfReadModel(CodeRenderingContext context) {
            ...
            return $$"""
                ...
                public virtual void {{AppSrvValidateMethod}}({{refSearchCondition.CsClassName}} refSearchConditionFilter, IPresentationContext context) {
                    // このメソッドをオーバーライドしてエラーチェック処理を記述してください。
                }
                /// <summary>
                /// {{_aggregate.Item.DisplayName}} が他の集約から参照されるときの検索結果カウント
                /// </summary>
                public virtual int {{AppSrvCountMethod}}({{refSearchCondition.CsFilterClassName}} refSearchConditionFilter, IPresentationContext context) {
                    ...
                    var querySource = {{load.AppSrvCreateQueryMethod}}(searchCondition, context);
                ...
                /// <returns>検索結果</returns>
                public virtual IEnumerable<{{refSearchResult.CsClassName}}> {{AppSrvLoadMethod}}({{refSearchCondition.CsClassName}} refSearchCondition, IPresentationContext context) {

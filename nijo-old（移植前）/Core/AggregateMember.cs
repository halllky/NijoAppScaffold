...
namespace Nijo.Core {
    internal class AggregateMemberNode : IGraphNode {
        ...
        public int? TotalDigits { get; set; } // 実数値の全体桁数
        public int? FractionalDigits { get; set; } // 実数値の少数部桁数
        ...
        /// <summary>マイナス値入力不可の制限属性</summary>
        public bool NotNegative { get; set; }

        public string? EnumSqlParamType { get; set; }

        public bool SearchConditionOnly { get; set; }
    ...
    public static class AggregateMember {
        ...
        /// <summary>
        /// この集約に属するメンバーを列挙します。
        /// <see cref="GetMembers(GraphNode{Aggregate}, bool)"/> よりも厳格にスキーマ定義上での順番を反映します。
        /// （主に外部キーが列挙される順番が違う）
        /// </summary>
        internal static IOrderedEnumerable<AggregateMemberBase> GetMembersOrderByStrict(
            this GraphNode<Aggregate> aggregate,
            bool isSearchCondition = false,
            E_GetMembersFlag? flags = null) {

            return aggregate
                .GetNonOrderedMembers(isSearchCondition, flags ?? E_GetMembersFlag.All)
                .OrderBy(member => {
                    if (member is not ValueMember vm) {
                        return member.Order;

                    } else if (vm.Inherits == null) {
                        return member.Order;

                    } else {
                        return vm.Inherits.Relation.GetMemberOrder();
                    }
                });
        }

        /// <summary>
        /// パフォーマンス改善用。どのメンバーが欲しいかのフラグ
        /// </summary>
        [Flags]
        internal enum E_GetMembersFlag {
            ValueMember = 1 << 0,
            Ref = 1 << 1,
            Parent = 1 << 2,
            Child = 1 << 3,

            All = ValueMember | Ref | Parent | Child,
        }

        /// <summary>
        /// この集約に属するメンバーを列挙します。
        /// </summary>
        /// <param name="flags">パフォーマンス改善用。指定すると特定の種類のメンバーのみ列挙される。</param>
        internal static IOrderedEnumerable<AggregateMemberBase> GetMembers(
            this GraphNode<Aggregate> aggregate,
            bool isSearchCondition = false,
            E_GetMembersFlag? flags = null) {
        ...
        private static IEnumerable<AggregateMemberBase> GetNonOrderedMembers(
            this GraphNode<Aggregate> aggregate,
            bool isSearchCondition,
            E_GetMembersFlag flags) {

            var parentEdge = aggregate.GetParent();
            if (parentEdge != null) {
                var parent = new Parent(parentEdge, aggregate);
                if ((flags & E_GetMembersFlag.Parent) != 0) {
                    yield return parent;
                }
                if ((flags & E_GetMembersFlag.ValueMember) != 0) {
            ...
            if ((flags & E_GetMembersFlag.ValueMember) != 0) {
                ...
                foreach (var edge in memberEdges) {
                    var memberNode = edge.Terminal.As<AggregateMemberNode>();

                    if (memberNode.Item.SearchConditionOnly && !isSearchCondition) continue;

                    yield return new Schalar(memberNode);
                }
            }

            if ((flags & E_GetMembersFlag.Child) != 0) {
            ...
            foreach (var edge in refEdges) {
                ...
                if ((flags & E_GetMembersFlag.Ref) != 0) {
        ...
        /// </summary>
        internal static IEnumerable<AggregateMemberBase> GetKeys(this GraphNode<Aggregate> aggregate, E_GetMembersFlag? flags = null) {
            foreach (var member in aggregate.GetMembers(flags: flags)) {
        ...
        internal class Parent : RelationMember {
            ...
            internal IEnumerable<ValueMember> GetForeignKeys() {
                foreach (var parentPk in Relation.Initial.GetKeys(E_GetMembersFlag.ValueMember)) {
                ...
                for (int i = 0; i < _settings._proxyMemberPath.Length; i++) {
                    ...
                    if (isLast) {
                        ...
                        if (_settings._proxyMemberPath.First() == AggregateMember.PARENT_PROPNAME
                            && currentAggregate.IsInEntryTree()) {

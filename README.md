...
### 4. 自動生成と手動実装の分離（ジェネレーションギャップパターン）
Nijoは「ジェネレーションギャップパターン」を採用しています。
自動生成されるコード（`__AutoGenerated` などといった名称のフォルダの中に生成されます）と、開発者が手動で実装するコードは、明確にファイルやフォルダが分離されています。

- **再生成しても安全**: スキーマ定義を変更してコードを再生成しても、あなたが実装したビジネスロジックやUIコンポーネントが上書きされて消えることはありません。
- **拡張可能な設計**: 生成されたクラスを継承したり、Partial Class（C#の場合）を利用したりすることで、自動生成コードに手を加えることなく振る舞いを拡張できます。
...
2から5までの工程は一度行なって終わりではなく、開発期間中を通じて継続的に行われます。3で生成されるソースコードと、4で人の手で実装されるソースコードは、ファイル単位で分離されるため、項目定義の変更があるたびにコード生成をかけなおすことで、この開発サイクルを実現しています。

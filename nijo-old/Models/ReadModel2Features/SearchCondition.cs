...
namespace Nijo.Models.ReadModel2Features {
    ...
    internal class SearchCondition {
        ...
        internal IEnumerable<SearchConditionMember> GetOwnMembers() {
            return _aggregate
                .GetMembers(isSearchCondition: true)
        ...
        internal virtual IEnumerable<DescendantSearchCondition> GetChildMembers() {
            foreach (var rm in _aggregate.GetMembers(isSearchCondition: true).OfType<AggregateMember.RelationMember>()) {
        ...
        internal IEnumerable<SearchConditionMember> EnumerateFilterMembersRecursively() {
            foreach (var member in GetOwnMembers()) {
                // 検索条件オンリーのメンバーのフィルタリング処理はカスタマイズで書かせる
                if (member.Member.Options.SearchConditionOnly) continue;
        ...
        private IEnumerable<SearchConditionMember> EnumerateSortMembers() {
            foreach (var scMember in GetOwnMembers()) {
                ...
                // 検索条件オンリーの項目ではソート不可
                if (scMember.Member.Options.SearchConditionOnly) continue;
        ...
        private static string GetSortLiteralWithoutAscDesc(SearchConditionMember member) {
            var fullpath = member.Member
                .GetFullPathAsSearchConditionFilter(E_CsTs.CSharp)
                .Skip(1); // "Filter"という名称を除外
        ...
        internal string TypeScriptSortableMemberType => $"SortableMemberOf{_aggregate.Item.PhysicalName}";
        internal string RenderTypeScriptSortableMemberType() {
            return $$"""
                /** {{_aggregate.Item.DisplayName}} のメンバーのうちソート可能なもの */
                export type {{TypeScriptSortableMemberType}}
                {{If(!EnumerateSortMembersRecursively().Any(), () => $$"""
                  = never
                """)}}
                {{EnumerateSortMembersRecursively().SelectTextTemplate((member, i) => $$"""
                  {{(i == 0 ? "=" : "|")}} '{{GetSortLiteralWithoutAscDesc(member).Replace("'", "\\'")}}'
                """)}}
                """;
        }
        #endregion ソート
        ...
        protected virtual string RenderTypeScriptDeclaring(CodeRenderingContext context) {
            ...
            var sortType = $"(`${{{TypeScriptSortableMemberType}}}{ASC_SUFFIX}` | `${{{TypeScriptSortableMemberType}}}{DESC_SUFFIX}`)[]";

...
namespace Nijo.Models.ReadModel2Features {
    ...
    internal class DataClassForDisplay {
        ...
        /// <summary>メッセージ用構造体 C#クラス名（ルート集約のみ）</summary>
        internal string MessageDataListCsClassName => $"{CsClassName}MessagesList";
        ...
        /// <summary>このデータを示す一意のID（C#）</summary>
        internal const string UNIQUE_ID_CS = "UniqueId";
        /// <summary>このデータを示す一意のID（TypeScript）</summary>
        ...
        private string RenderCsMessageClass(CodeRenderingContext context) {
            ...
            return $$"""
                ...
                /// </summary>
                public partial class {{MessageDataListCsClassName}} : {{DisplayMessageContainer.CONCRETE_CLASS_LIST}}<{{MessageDataCsClassName}}> {
                    public {{MessageDataListCsClassName}}(IEnumerable<string> path) : base(path, i => new([.. path, i.ToString()])) { }

                    /// <summary>すべてのメッセージを画面ルートに転送する場合に用いられるコンストラクタ</summary>
                    public {{MessageDataListCsClassName}}({{DisplayMessageContainer.INTERFACE}} origin) : base(origin, _ => new(origin)) { }
                }
                ...
                {{members.SelectTextTemplate(m => $$"""
                        {{WithIndent(RenderConstructor(m.MemberInfo, false), "        ")}}
                """)}}
                    }
                    /// <summary>すべてのメッセージを画面ルートに転送する場合に用いられるコンストラクタ</summary>
                    public {{MessageDataCsClassName}}({{DisplayMessageContainer.INTERFACE}} origin) : base(origin) {
                {{members.SelectTextTemplate(m => $$"""
                        {{WithIndent(RenderConstructor(m.MemberInfo, true), "        ")}}
                """)}}
                    }
            ...
            string RenderConstructor(AggregateMember.AggregateMemberBase member, bool isTransferRendering) {
                if (member is AggregateMember.ValueMember || member is AggregateMember.Ref) {
                    if (isTransferRendering) {
                        return $$"""
                    ...
                    if (isTransferRendering) {
                        return $$"""
                            {{member.MemberName}} = new([], rowIndex => {
                                return new {{desc.MessageDataCsClassName}}(origin);
                            });
                            """;
                    } else if (children.ChildrenAggregate.CanDisplayAllMembersAs2DGrid()) {
                        return $$"""
                    ...
                    if (isTransferRendering) {
                        return $$"""
                            {{member.MemberName}} = new {{desc.MessageDataCsClassName}}(origin);
                            """;
                    } else {
                        return $$"""
        ...
        private string RenderDeepEqualFunction(CodeRenderingContext context) {
            ...
            return $$"""
                ...
                export const {{DeepEqualFunction}} = (a: {{TsTypeName}}, b: {{TsTypeName}}): boolean => {
                  ...
                  if (a.{{DataClassForDisplay.WILL_BE_DELETED_TS}} !== b.{{DataClassForDisplay.WILL_BE_DELETED_TS}}) return false
                  return true
                }
                """;
            ...
            string RenderAggregate(DataClassForDisplay rendering, string a, string b, GraphNode<Aggregate> instanceAgg) {
                ...
                string RenderOwnMember(AggregateMember.AggregateMemberBase member) {
                    if (member is AggregateMember.ValueMember vm) {
                        ...
                        if (vm.Options.MemberType is FileAttachment) {
                            var hash = vm.Options.Id.ToString().ToHashedString().Substring(0, 4);
                            return $$"""

                            // 添付ファイルのディープイコールはIDで比較
                            const files1_{{hash}} = {{a}}.{{fullpath.Join("?.")}} ?? []
                            const files2_{{hash}} = {{b}}.{{fullpath.Join("?.")}} ?? []
                            if (files1_{{hash}}.length !== files2_{{hash}}.length) return false
                            const files2Set_{{hash}} = new Set(files2_{{hash}}.map(f2 => f2.{{FileAttachment.C_FILE_ATTACHMENT_ID}}))
                            for (let f1 of files1_{{hash}}) {
                              if (!files2Set_{{hash}}.has(f1.{{FileAttachment.C_FILE_ATTACHMENT_ID}})) return false
                            }

                            """;
                        } else if (vm.Options.MemberType is YearMonth
                                || vm.Options.MemberType is YearMonthDay
                                || vm.Options.MemberType is YearMonthDayTime) {

                            // 日付時刻はdayjsで比較。実際の比較ロジックは自動生成されず生のJavaScriptコード側で実装
                            return $$"""
                                if (Util.toISOStringStrict({{a}}.{{fullpath.Join("?.")}}) !== Util.toISOStringStrict({{b}}.{{fullpath.Join("?.")}})) return false
                                """;

                        } else if (vm.Options.MemberType is Integer
                                || vm.Options.MemberType is Numeric) {

                            // 数値はdecimal.jsで比較。実際の比較ロジックは自動生成されず生のJavaScriptコード側で実装
                            return $$"""
                                if (!Util.strictDecimalEquals({{a}}.{{fullpath.Join("?.")}},{{b}}.{{fullpath.Join("?.")}})) return false
                                """;
        ...
        private string RenderConvertFromSearchResultPrivate(
            ...
            return $$"""
                ...
                {{If(HasLifeCycle, () => $$"""
                    ...
                    {{UNIQUE_ID_CS}} = Guid.NewGuid().ToString(),
                """)}}
                {{If(HasVersion, () => $$"""

...
namespace Nijo.Models {
    ...
    internal class WriteModel2 : IModel {
        void IModel.GenerateCode(CodeRenderingContext context, GraphNode<Aggregate> rootAggregate) {
            ...
            aggregateFile.AppServiceMethods.Add(NotNegativeCheck.Render(rootAggregate, context));
            aggregateFile.AppServiceMethods.Add(CharacterTypeCheck.Render(rootAggregate, context));
            aggregateFile.AppServiceMethods.Add(DigitsCheck.Render(rootAggregate, context));
            aggregateFile.AppServiceMethods.Add(DynamicEnum.RenderAppSrvCheckMethod(rootAggregate, context));

            // 処理: シーケンス採番
            aggregateFile.AppServiceMethods.Add(new GenerateAndSetSequenceMethod(rootAggregate).RenderAppSrvMethod(context));
            ...
            // データ型: メタ情報
            context.UseSummarizedFile<WriteModelMetadata>().Register(rootAggregate);
        }

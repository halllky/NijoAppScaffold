        ...
        private static IEnumerable<string> RenderAggregate(GraphNode<Aggregate> renderingAggregate, string instance, GraphNode<Aggregate> instanceAggregate) {
            foreach (var member in renderingAggregate.GetMembers()) {
                ...
                if (member is AggregateMember.Schalar schalar) {
                    ...
                    if (schalar.Options.MemberType is SequenceMember) continue;     //シーケンスの時は登録処理で採番されるので不要
                    ...
                    yield return $$"""
                        if ({{isEmpty}}) {
                            e.{{GetErrorMemberPath(member).Join(".")}}.AddError(MSG.ERRC0003("{{memberDisplayName}}", string.Empty));
                        }
                        """;
                    ...
                    yield return $$"""
                        if ({{WithIndent(isEmpty, "    || ")}}) {
                            e.{{GetErrorMemberPath(@ref).Join(".")}}.AddError(MSG.ERRC0003("{{memberDisplayName}}", string.Empty));
                        }
                        """;
                    ...
                    yield return $$"""
                        if ({{instance}}.{{switchPath.Join("?.")}} == null) {
                            e.{{GetErrorMemberPath(variation).Join(".")}}.AddError(MSG.ERRC0003("{{memberDisplayName}}", string.Empty));
                        }
                        """;

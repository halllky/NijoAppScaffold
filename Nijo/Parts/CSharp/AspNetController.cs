        ...
        internal static SourceFile RenderPresentationContextModelBinder(CodeRenderingContext ctx) {
            return new SourceFile {
                ...
                Contents = $$"""
                    ...
                    /// <summary>
                    /// PresentationContext のモデルバインディング（戻り値なし）
                    /// </summary>
                    public sealed class {{PRESENTATION_CONTEXT_BINDER}}<TMessageRoot> : IModelBinder
                    ...
                    /// <summary>
                    /// PresentationContext のモデルバインディング（戻り値あり）
                    /// </summary>
                    public sealed class {{PRESENTATION_CONTEXT_BINDER}}<TReturnValue, TMessageRoot> : IModelBinder
                        where TMessageRoot : {{MessageContainer.SETTER_INTERFACE}}
                        ...
                        public async Task BindModelAsync(ModelBindingContext bindingContext) {
                            ...
                            // 詳細なロジックは自動生成されないメソッドに譲る
                            var presentationContext = await configuration.{{INITIALIZE_PRESENTATION_CONTEXT}}<TReturnValue, TMessageRoot>(bindingContext, actionMetadata);
        ...
        internal static void RegisterWebapiConfiguration(IMultiAggregateSourceFileManager ctx) {
            ctx.Use<ApplicationConfigure>()
                ...
                .AddWebapiMethod($$"""
                    ...
                    /// <param name="metadata">NijoAppScaffoldが提供するメタ情報</param>
                ...
                .AddWebapiMethod($$"""
                    ...
                    /// <param name="bindingContext">ASP.NET Core が提供するモデルバインディングの仕組みが提供する情報。</param>
                    /// <param name="metadata">NijoAppScaffoldが提供するメタ情報</param>
                    public abstract Task<{{PresentationContext.INTERFACE}}<TMessageRoot>> {{INITIALIZE_PRESENTATION_CONTEXT}}<TMessageRoot>(ModelBindingContext bindingContext, {{AUTOGENERATED_ACTION_METADATA}} metadata)
                        where TMessageRoot : {{MessageContainer.SETTER_INTERFACE}};
                    """)
                .AddWebapiMethod($$"""
                    /// <summary>
                    /// ComplexPostリクエストの開始時に呼ばれる。
                    /// <see cref="{{PresentationContext.INTERFACE_WITH_RETURN_VALUE}}{TReturnValue, TMessageRoot}"/> を具備したクラスのインスタンスを初期化して返す。
                    /// </summary>
                    /// <typeparam name="TReturnValue">戻り値の型</typeparam>
                    /// <typeparam name="TMessageRoot">パラメータのデータ型（≒画面のデータ型）と対応するエラーメッセージの入れ物の型</typeparam>
                    /// <param name="bindingContext">ASP.NET Core が提供するモデルバインディングの仕組みが提供する情報。</param>
                    /// <param name="metadata">NijoAppScaffoldが提供するメタ情報</param>
                    public abstract Task<{{PresentationContext.INTERFACE_WITH_RETURN_VALUE}}<TReturnValue, TMessageRoot>> {{INITIALIZE_PRESENTATION_CONTEXT}}<TReturnValue, TMessageRoot>(ModelBindingContext bindingContext, {{AUTOGENERATED_ACTION_METADATA}} metadata)
                        where TMessageRoot : {{MessageContainer.SETTER_INTERFACE}}
                        where TReturnValue : new();
                    """)
                .AddWebapiMethod($$"""

...
namespace Nijo.Models.QueryModelModules {
    internal class SearchProcessingRefs {
        ...
        internal string RenderAspNetCoreControllerAction(CodeRenderingContext ctx) {
            ...
            var returnType = $"{SearchProcessingReturn.TYPE_CS}<{new DisplayDataRef.Entry(_entryAggregate).CsClassName}>";
            ...
            return $$"""
                ...
                public async Task<{{SearchProcessingReturn.TYPE_CS}}<{{displayData.CsClassName}}>> {{LoadMethod}}({{searchCondition.CsClassName}} searchCondition, {{PresentationContext.INTERFACE}}<{{searchConditionMessage.CsClassName}}> context) {
                    ...
                    try {
                        ...
                        loaded = await converted.ToArrayAsync().ConfigureAwait(false);
                    } catch {
                ...
                foreach (var member in displayDataInstance.CreateProperties()) {
                    ...
                    if (member is InstanceValueProperty vp) {
                        ...
                        if (sp.Metadata.IsArray) {
                            ...
                            var childrenMetadata = (IInstancePropertyOwnerMetadata)sp.Metadata;
                            var left = new Variable("newステートメントなので変数名なし", childrenMetadata);

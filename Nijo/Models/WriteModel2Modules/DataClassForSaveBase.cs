...
namespace Nijo.Models.WriteModel2Modules {
    ...
    internal class DataClassForSaveBase : IMultiAggregateSourceFile {
        ...
        void IMultiAggregateSourceFile.RegisterDependencies(IMultiAggregateSourceFileManager ctx) {
            var requiresAddModDelEnum = ctx.Schema.GetRootAggregates().Any(rootAggregate => rootAggregate.Model is Models.ReadModel2 || rootAggregate.GenerateDefaultQueryModel);
            if (requiresAddModDelEnum) {
            ...
            ctx.ReactProject(autoGenerated => {
                autoGenerated.Directory("util", utilDir => {
                    utilDir.Generate(new SourceFile {
                        ...
                        Contents = RenderTypeScriptBaseTypes(ctx),
                    });
                });
            });
        }
        ...
        private string RenderTypeScriptBaseTypes(CodeRenderingContext ctx) {
            if (ctx.IsLegacyCompatibilityMode()) {
                return $$"""
                    ...
                    export const {{GET_ADD_MOD_DEL_ENUM_TS}} = (item: {
                      addModDel?: {{ADD_MOD_DEL_ENUM_TS}}
                      existsInDatabase?: boolean
                      willBeChanged?: boolean
                      willBeDeleted?: boolean
                    }): {{ADD_MOD_DEL_ENUM_TS}} => {
                      // 明示的に指定されている場合はそれを優先
                      if (item.addModDel) return item.addModDel
                      // DBに無いデータならば新規追加データ
                      if (!item.existsInDatabase) {
                        return item.willBeDeleted

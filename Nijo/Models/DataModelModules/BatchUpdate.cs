        ...
        internal string RenderControllerAction(CodeRenderingContext ctx) {
            ...
            return $$"""
                ...
                public async Task<IActionResult> BatchUpdate(
                    ...
                    [ModelBinder(BinderType = typeof({{AspNetController.PRESENTATION_CONTEXT_BINDER}}<{{MessageContainer.SETTER_CONCRETE_CLASS_LIST}}<{{displayDataMessages.CsClassName}}>>))] {{PresentationContext.INTERFACE}}<{{MessageContainer.SETTER_CONCRETE_CLASS_LIST}}<{{displayDataMessages.CsClassName}}>> {{AspNetController.CONTEXT}}) {

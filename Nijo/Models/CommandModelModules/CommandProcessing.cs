...
namespace Nijo.Models.CommandModelModules {
    ...
    internal class CommandProcessing {
        ...
        internal string RenderAspNetCoreControllerAction(CodeRenderingContext ctx) {
            var paramTypeName = _rootAggregate.GetParameterStructure()?.CsClassName ?? "object";
            ...
            return $$"""
                ...
                public async Task<IActionResult> Execute(
                    ...
                    [ModelBinder(BinderType = typeof({{AspNetController.PRESENTATION_CONTEXT_BINDER}}<{{MessageContainer.CONCRETE_CLASS}}>))] {{PresentationContext.INTERFACE}}<{{MessageContainer.CONCRETE_CLASS}}> {{AspNetController.CONTEXT}}) {
        ...
        internal string RenderAppSrvMethods(CodeRenderingContext ctx) {
            var paramTypeName = _rootAggregate.GetParameterStructure()?.CsClassName ?? "object";
            var returnTypeName = _rootAggregate.GetReturnValueStructure()?.CsClassName ?? "object";
            ...
            return $$"""
                ...
                public abstract Task<{{returnTypeName}}> {{EXECUTE_METHOD}}({{paramTypeName}} param, {{PresentationContext.INTERFACE}}<{{MessageContainer.CONCRETE_CLASS}}> context);
                """;
        }
    }
}

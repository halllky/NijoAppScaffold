...
function EmployeeGrid({ methods }: { methods: UseFormReturn<従業員一括更新ParameterDisplayData> }) {
  ...
  const handleRemove = (index: number) => {
    ...
    if (item.existsInDatabase) {
      ...
      // useFieldArrayのupdate機能を使うか、setValueを使う
      const current = methods.getValues(`更新対象従業員一覧.${index}`)
      // willBeDeletedを反転のみでなく、trueにする
      // ここでは表示上消すのではなく、取り消し線などを出すか、あるいは「削除」状態にするUIが必要
      // 簡易的に、削除ボタンを押したら willBeDeleted = true にする

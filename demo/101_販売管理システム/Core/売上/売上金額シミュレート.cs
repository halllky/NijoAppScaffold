...
partial class OverridedApplicationService {
    ...
    public static void Simulate(売上詳細DisplayData param) {
        ...
        foreach (var item in param.売上詳細の売上明細) {
            ...
            // 丸め処理は明細単位
            var taxRate = item.Values.商品?.消費税区分 switch {
                消費税区分.一般税率 => 1.10m,
                消費税区分.軽減税率 => 1.08m,
                _ => 1.00m,
            };
            var multiplied = (item.Values.売上数量 ?? 0m) * (item.Values.商品?.売値単価_税抜 ?? 0m) * taxRate;
            var rounded = Math.Round(multiplied, 0, MidpointRounding.AwayFromZero);

            // 取消（赤伝）の場合は負の金額
            item.Values.売上総額_税込_自動計算 = item.Values.区分 == 売上明細区分.売上
                ? (int)rounded
                : -(int)rounded;
        }

        param.Values.合計金額 = param.売上詳細の売上明細.Sum(d => d.Values.売上総額_税込_手修正
                                                          ?? d.Values.売上総額_税込_自動計算
                                                          ?? 0);
    }

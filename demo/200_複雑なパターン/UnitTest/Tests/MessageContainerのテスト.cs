...
internal class MessageContainerのテスト {
    ...
    public void ToJsonObjectの基本的なテスト() {
        ...
        // ---------------------
        // Act
        var json = CommandQueryControllerHandler.ToJsonObject(context.Find([])!);
        ...
        Assert.That(json["children"]!.AsObject().ContainsKey("機器詳細"), Is.True, "機器詳細キーが存在しません");
        var 機器詳細Json = json["children"]!.AsObject()["機器詳細"]!.AsObject();

        // 子配列のキーが存在することを確認
        Assert.That(機器詳細Json["children"]!.AsObject().ContainsKey("付属品"), Is.True, "付属品キーが存在しません");
        var 付属品Json = 機器詳細Json["children"]!.AsObject()["付属品"]!.AsObject(); // 配列もJsonObjectとして扱われる

        Assert.That(付属品Json["children"]!.AsObject().ContainsKey("3"), Is.True, "付属品[3]が存在しません");
        var 付属品3Json = 付属品Json["children"]!.AsObject()["3"]!.AsObject();

        Assert.That(付属品3Json["children"]!.AsObject().ContainsKey("付属品名"), Is.True, "付属品[3].付属品名が存在しません");
        var 付属品名Json = 付属品3Json["children"]!.AsObject()["付属品名"]!.AsObject();
    ...
    public void 異なる添字の配列要素にメッセージがある場合() {
        ...
        // Assert
        // 機器詳細が存在することを確認
        var root = json["children"]!.AsObject()["ルート"]!.AsObject()["children"]!.AsObject()["医療機器"]!.AsObject();
        Assert.That(root["children"]!.AsObject().ContainsKey("機器詳細"), Is.True, "機器詳細キーが存在しません");
        var 機器詳細Json = root["children"]!.AsObject()["機器詳細"]!.AsObject();
        // 付属品が存在することを確認
        Assert.That(機器詳細Json["children"]!.AsObject().ContainsKey("付属品"), Is.True, "付属品キーが存在しません");
        var 付属品Json = 機器詳細Json["children"]!.AsObject()["付属品"]!.AsObject();

        // インデックス1が存在することを確認
        Assert.That(付属品Json["children"]!.AsObject().ContainsKey("1"), Is.True, "付属品[1]が存在しません");
        var 付属品1Json = 付属品Json["children"]!.AsObject()["1"]!.AsObject();
        Assert.That(付属品1Json["children"]!.AsObject().ContainsKey("数量"), Is.True, "付属品[1].数量が存在しません");
        var 数量1Json = 付属品1Json["children"]!.AsObject()["数量"]!.AsObject();
        Assert.That(数量1Json.ContainsKey("error"), Is.True, "付属品[1].数量.errorが存在しません");
        Assert.That((string)数量1Json["error"]![0]!, Is.EqualTo("数量1のエラー"));

        // インデックス5が存在することを確認
        Assert.That(付属品Json["children"]!.AsObject().ContainsKey("5"), Is.True, "付属品[5]が存在しません");
        var 付属品5Json = 付属品Json["children"]!.AsObject()["5"]!.AsObject();
        Assert.That(付属品5Json["children"]!.AsObject().ContainsKey("数量"), Is.True, "付属品[5].数量が存在しません");
        var 数量5Json = 付属品5Json["children"]!.AsObject()["数量"]!.AsObject();
        ...
        Assert.That(付属品Json["children"]!.AsObject().ContainsKey("2"), Is.False, "付属品[2]が存在しています");
    }
    ...
    public void 複数種類のメッセージをひとつのプロパティに追加できること() {
        ...
        // ---------------------
        // Assert
        Assert.That(json["children"]!.AsObject().ContainsKey("機器名"), Is.True, "機器名キーが存在しません");
        var 機器名Json = json["children"]!.AsObject()["機器名"]!.AsObject();
    ...
    public void 複数のエラーメッセージを同じプロパティに追加できること() {
        ...
        // ---------------------
        // Assert
        Assert.That(json["children"]!.AsObject().ContainsKey("単価"), Is.True, "単価キーが存在しません");
        var 単価Json = json["children"]!.AsObject()["単価"]!.AsObject();
    ...
    public void 様々な階層のオブジェクトにメッセージを追加できること() {
        ...
        // 直接の子要素のエラー確認
        Assert.That(json["children"]!.AsObject().ContainsKey("機器名"), Is.True, "機器名キーが存在しません");
        Assert.That((string)json["children"]!["機器名"]!["error"]![0]!, Is.EqualTo("機器名の値が異常です。"));
        ...
        // インデックス2の要素確認
        Assert.That(付属品Json["children"]!.AsObject().ContainsKey("2"), Is.True, "付属品[2]が存在しません");
        Assert.That((string)付属品Json["children"]!["2"]!["children"]!["付属品名"]!["error"]![0]!, Is.EqualTo("付属品名が未入力です。"));

        // インデックス7の要素確認
        Assert.That(付属品Json["children"]!.AsObject().ContainsKey("7"), Is.True, "付属品[7]が存在しません");
        Assert.That((string)付属品Json["children"]!["7"]!["children"]!["数量"]!["warn"]![0]!, Is.EqualTo("数量が標準的ではありません。"));
    }

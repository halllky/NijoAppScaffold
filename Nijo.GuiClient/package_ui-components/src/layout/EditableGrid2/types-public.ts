...
export type EditableGrid2Props<TRow> = {
   ...
   * 行データ。
   * パフォーマンスの最適化のため、
   * 配列を直接バインドする方式と、各ボディセル自身が値の状態を管理する方式の2種類を用意しています。
   *
   * 1. 配列を直接指定した場合は行オブジェクト自体の変更を検知して再レンダリングしているため、
   *    **各行のプロパティだけでなく行オブジェクト自体も新しいオブジェクトに置き換える必要があります。**
   * 2. オブジェクトで指定した場合は、トータルの行数を検知するための配列と、
   *    行インデックスからその行の最新の値を取得する関数を指定します。
   *    これにより、行オブジェクト自体が変更されていなくても、関数から取得した最新の値が使用されるため、
   *    **行オブジェクトのプロパティだけを更新すれば反映されます。**
   *
   * 大量の行データを扱う場合や、React Hook Formなどのフォームライブラリと組み合わせて使用する場合は
   * 2. の方式を使用することを推奨します。
   */
  rows: TRow[] | {
    /**
     * 行データの配列。全体として何行あるかの検知のみに使用されるため、
     * 行数が実際のデータの数と一致していれば実際のデータが入っていなくても構いません。
     */
    array: unknown[]
    /**

...
export const useForeignKeyLookup = ({
  ...
  refToAggregate: MetadataOfEFCoreEntity.Aggregate | null
  ...
  React.useEffect(() => {
    ...
    const foreignKeyMembers = ownerTableMetadata.members.filter(
      (m): m is MetadataOfEFCoreEntity.AggregateMember =>
        (m.type === "ref-key" || m.type === "ref-parent-key") &&
        (m as MetadataOfEFCoreEntity.AggregateMember).refToRelationName === foreignKeyRelationName
    )

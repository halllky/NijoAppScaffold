...
export const useDbRecordGridColumnDef = (
  ...
  tableMetadata: MetadataOfEFCoreEntity.Aggregate | undefined,
      ...
      if (column.type === "ref-key" || column.type === "own-column" || column.type === "parent-key" || column.type === "ref-parent-key") {
        ...
        if (isLastOfSameRefToRelationName) {
          ...
          for (const additionalColumnName of additionalColumnNames ?? []) {
            const refColumn = refToAggregate.members.find(m => m.columnName === additionalColumnName) as MetadataOfEFCoreEntity.AggregateMember | undefined
            if (!refColumn) {
...
export type ForeignKeyReferenceDialogProps = {
  tableMetadata: MetadataOfEFCoreEntity.Aggregate
...
const RefColumnCellRenderer = ({
  ...
  foreignKeyColumn: MetadataOfEFCoreEntity.AggregateMember
  refColumn: MetadataOfEFCoreEntity.AggregateMember
  refToAggregate: MetadataOfEFCoreEntity.Aggregate
  ownerTableMetadata: MetadataOfEFCoreEntity.Aggregate
}) => {
  const { getDbRecords } = useQueryEditorServerApi()

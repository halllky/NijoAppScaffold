...
export type MentionBaseEditableProps = {
  value?: string
  ...
  onClickMention?: (part: StringPart) => void
  /** メンション候補データを取得する関数 */
 ...
 */
export const MentionBaseEditable = React.forwardRef<HTMLTextAreaElement, MentionBaseEditableProps>(({
  value,
  onChange,
  onKeyDown,
  placeholder,
 ...
 */
export function MentionBaseReadOnly({
  title,
  children,
  className,
  style,
  ...
  const handleClickMention = useEvent((part: StringPart) => {
    onClickMention?.(part)
  })

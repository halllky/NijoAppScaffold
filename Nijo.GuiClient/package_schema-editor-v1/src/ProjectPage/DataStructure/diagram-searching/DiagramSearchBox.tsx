 ...
 * 検索対象はルート集約とその子孫の物理名、コメント、テキスト値、文字列型・数値型の属性の値。
 ...
 */
export function DiagramSearchBox({ formMethods, onHitRootIdsChanged, className }: {
  ...
  const search = (target: SearchCondition) => {
    onHitRootIdsChanged(searchRootAggregates(
      getValues('dataStructures'),
      getValues('commands'),
      attributeDefs,
      getValues('customAttributes'),
      target,
    ))
  }

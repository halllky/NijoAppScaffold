...
import { buildDiagramStructure, type DiagramStructure } from "./diagramStructure"
 ...
 */
export function DiagramStructureProvider({ formMethods, children }: {
  ...
  // ダイアグラムの構成
  const [structure, setStructure] = React.useState(() => buildDiagramStructure(getValues()))

  /** フォームの最新の値からダイアグラムの構成を組み立て直す */
  const notifyStructureChanged = React.useCallback(() => {
    setStructure(buildDiagramStructure(getValues()))
  }, [getValues])

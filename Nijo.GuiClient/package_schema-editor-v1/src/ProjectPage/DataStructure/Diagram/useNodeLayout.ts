 ...
 */
export function useNodeLayout(formMethods: ReactHookForm.UseFormReturn<EditingProject>, roots: DiagramAggregate[], references: DiagramReference[]) {

  const { control, setValue } = formMethods

  // ユーザーがドラッグして動かしたノードの位置。
  // graphViewState はサーバーに nijo.viewState.json が無い場合 null になりうるため、
  // 深い階層への setValue で中間オブジェクトの補完に頼らずに済むよう、フィールド全体を watch/書き換えする
  const graphViewState = ReactHookForm.useWatch({ control, name: 'graphViewState' })
  const movedPositions = graphViewState?.schemaDefinition?.nodePositions ?? EMPTY_POSITIONS
  ...
  const handleNodesChange = (changes: NodeChange<Node>[]) => {
    ...
    if (Object.keys(moved).length > 0) {
      setValue('graphViewState', { schemaDefinition: { nodePositions: { ...movedPositions, ...moved } } }, { shouldDirty: true })
...
/** movedPositions が未定義のときに毎回同じオブジェクトを使うための空オブジェクト */
const EMPTY_POSITIONS: { [id: string]: XYPosition } = {}
